---
title: "Visualizing genomic rearrangements using the R package *GenomicRearrangementPlot*"
author: "José Espejo Valle-Inclán and Isidro Cortes-Ciriano*"
date: "07/05/2022"
...
Here, we present the R package *GenomicRearrangementPlot*, which provides functionalities for the visualization of copy number profiles and SVs. The package relies on public packages, including the popular ggplot2, thus allowing further customization of the plots.

To visualize genomic rearrangement profiles, *GenomicRearrangementPlot* requires structural variation and copy number data in the commonly used bed format. For SVs, *GenomicRearrangementPlot* uses breakend notation to facilitate compatibility with any SV caller conforming with best practices for reporting SVs in vcf format.
While the primary focus of *GenomicRearrangementPlot* is the visualization of complex somatic copy number and SVs in the context of cancer, the functionalities provided can be used to visualize genomics data from non-cancer samples, such as somatic aberrations detected in normal tissues, single-cell whole-genome data or others. However, note that at present *GenomicRearrangementPlot* only supports the build of the human reference genome GRCh38. 

## Examples
In the next section we provide examples of the functionalities of *GenomicRearrangementPlot* and provide best practices to generate high-quality plots.
First, we load the package.
```{r}
library(GenomicRearrangementPlot)
library(ggplot2)
```
...
The main function of the package is *rearrangement_plot*. This function generates a rearrangement plot for the genomic regions defined above in the *chr_selection* data.frame. 
...
```{r, echo=TRUE, fig.width = (6/2.4), fig.height=(5/2.4), fig.align='center'}
plot = rearrangement_plot( sv_data,
cn_data,
...
```{r, echo=TRUE , fig.width = (7/2.4), fig.height=(5/2.4), fig.align='center'}
chrs=c("chr10")
chr_selection = data.frame(
  chr=chrs,
...
```{r, echo=TRUE, fig.width = (19/2.4), fig.height=(6.5/2.4), fig.align='center'}
plot = rearrangement_plot( sv_data,
cn_data,
chr_selection=chr_selection,
...
```{r, echo=TRUE, fig.width = (19/2.4), fig.height=(5/2.4), fig.align='center'}
plot = rearrangement_plot( sv_data,
cn_data,
chr_selection=chr_selection,
genes=c("MDM2","MYC","TP53","CDK4","EGFR"),
...
```{r, eval=FALSE, echo=TRUE, fig.width = (19/2.4), fig.height=(5/2.4), fig.align='center'}
pdf("example_rearrangement_plot.pdf", width=(19/2.4), height=c(5/2.4))
p
dev.off()
```
